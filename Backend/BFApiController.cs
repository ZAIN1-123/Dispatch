...
namespace DISPATCHAPI.Controllers
{
    ...
    public class BFApiController : Controller
    {
        private IConfiguration Configuration;
        private readonly ILogger<BFApiController> logger;
        public BFApiController(ILogger<BFApiController> _logger, IConfiguration _Configuration)
        {
            _logger = logger;
        ...
        public ActionResult Getch()
        {
            try
            {
                ...
                {
                    lst = conn.Query<BF>("select * from BF").ToList();
        ...
        public IActionResult Get(int id)
        {
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(ConnString))
                {
                    ...
                    lst = conn.Query<BF>("select * from BF where Id=" + id).FirstOrDefault();
        ...
        [HttpPost]
        public IActionResult Post(BF lst)
        {
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(ConnString))
                {
                    ...
                    if (user.BF == 1)
                    {
                        lst.Id = conn.ExecuteScalar<int>("select max(Id) from BF") + 1;

                        conn.Execute("insert into BF(Id,Name)values (@Id,@Name)", lst);
        ...
        public IActionResult put(int id, BF lst)

        {
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(ConnString))
                {
                    ...
                    {
                        conn.Execute("update BF set Name=@Name where Id=" + id, lst);
                       return Ok("Updated");
...

        {
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(ConnString))
                {
                    ...
                    {
                        conn.Execute("Delete from BF where Id=" + id);
